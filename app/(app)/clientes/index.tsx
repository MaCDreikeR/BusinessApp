...
export default function ClientesScreen() {
  ...
  const carregarClientes = async (forceRefresh = false) => {
    try {
      ...
      // Buscar todas as movimenta��es de credi�rio
      // A tabela crediario_movimentacoes n�o tem estabelecimento_id, 
      // ent�o vamos buscar s� dos clientes deste estabelecimento
      ...
      if (clienteIds.length > 0) {
        ...
        if (movimentacoesError) {
          logger.error('Erro ao carregar movimenta��es:', movimentacoesError);
        } else {
  ...
  const abrirWhatsApp = (telefone: string | null | undefined, clienteId?: string, clienteNome?: string) => {
    ...
    if (!telefone || telefone.trim() === '') {
      Alert.alert(
        ...
        'Este cliente n�o possui telefone cadastrado. Deseja cadastrar agora?',
        [
          {
            text: 'Cancelar',
            style: 'cancel'
    ...
    if (numeroLimpo.length < 10) {
      Alert.alert(
        'Telefone inv�lido',
        'O telefone cadastrado � inv�lido. Deseja corrigir?',
        [
          {
            text: 'Cancelar',
            style: 'cancel'
  ...
  const importarContato = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permiss�o necess�ria', 'Precisamos de acesso aos seus contatos para import�-los.');
        return;
      }
      ...
      if (data.length === 0) {
        Alert.alert('Nenhum contato', 'N�o foram encontrados contatos no dispositivo.');
        return;
      }
      ...
      if (contatosValidos.length === 0) {
        Alert.alert('Nenhum contato v�lido', 'N�o foram encontrados contatos com nome e n�mero de telefone v�lidos.');
        return;
      }
      ...
      Alert.alert('Erro', 'N�o foi poss�vel acessar os contatos do dispositivo.');
    }
  };
      ...
      <View style={styles.filtrosWrapper}>
        ...
        >
          ...
          >
            ...
            <Text style={[styles.filtroTexto, filtro === 'com_credito' && styles.filtroTextoAtivo]}>Com cr�dito</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[styles.filtroTexto, filtro === 'com_debito' && styles.filtroTextoAtivo]}>Com D�bito</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
