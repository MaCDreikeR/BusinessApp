...
export default function ClientesScreen() {
  ...
  const carregarClientes = async (forceRefresh = false) => {
    try {
      ...
      // Encapsular queries com timeout de 60 segundos
      const clientesData = await withTimeout(
        supabase
          .from('clientes')
          .select('*')
      ...
      const agendamentosData = await withTimeout(
        supabase
          .from('agendamentos')
          .select('*')
          .eq('estabelecimento_id', estabelecimentoId)
      ...
      if (clienteIds.length > 0) {
        movimentacoesData = await withTimeout(
          supabase
